...
%% Directional CIR for each channel snapshot,此部分与MIMO无关！！
DirPDPInfo = [];
for i_snap = 1:numberOfSnapshot
    CIR_tmp = CIR_SISO_EVO.(['Snapshot',num2str(i_snap)]);
...
if plotStatus == true
    ...
    h1 = plotSFMap(area,sfMap,track,sceType,envType,SF,movDistance,velocity,f,dini,FigVisibility);
    %%% Plot 2: Spatially correlated LOS/NLOS map
    h2 = plotLOSMap(area,losMap,track,sceType,envType,movDistance,velocity,f,dini,FigVisibility);
    %%% Plot 3: User track
...
fid = fopen(file_name,'wt');
fprintf(fid, '%12s\t%12s\t%12s\t%12s\t%12s\t',...
    'Snapshot','T-R Separation Distance (m)','Received Power (dBm)','Path Loss (dB)','RMS Delay Spread (ns)'); 
fprintf(fid,'\n%8d\t%25.1f\t%15.1f\t%15.1f\t%20.1f',DirPDPInfo.'); 
fclose(fid);
